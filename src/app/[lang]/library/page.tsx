...
export default function ContentLibraryPage() {
  ...
  const handleDownload = (creator: string, size: string) => {
    toast.success("Téléchargement démarré", {
      description: `Téléchargement du brouillon de livraison MP4 haute résolution de ${creator} (${size}).`,
    });
  };

  const handleCopyLink = (link: string) => {
    navigator.clipboard.writeText(link);
    toast.success("L'URL de la publication a été copiée dans le presse-papiers", {
      description: link,
    });
  };
  ...
  return (
    <DashboardLayout>
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-muted-foreground/60">Ressources créatives</p>
          <h1 className="font-heading mt-2 text-[28px] font-bold tracking-tight text-foreground md:text-[34px]">Bibliothèque de contenu</h1>
          <p className="mt-1 text-[13px] text-muted-foreground">Accédez et téléchargez des livrables créatifs haute résolution approuvés à partir de campagnes.</p>
        </div>
      </div>
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3 justify-between bg-white/90 border border-border/40 p-4 rounded-3xl shadow-card">
        <div className="flex flex-wrap items-center gap-3 flex-1 min-w-[280px]">
          <div className="relative flex-1 max-w-[320px]">
            ...
            <input 
              ...
              placeholder="Rechercher du contenu de créateur…"
        ...
        <div className="text-[12px] text-muted-foreground font-semibold">
          Affichage de {filtered.length} sur {libraryItems.length} vidéos
        </div>
      </div>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="p-5 space-y-4">
              <div>
                ...
                <p className="text-xs text-foreground mt-1 font-medium flex items-center gap-1">
                  <CheckCircle2 className="h-3.5 w-3.5 text-success shrink-0" /> Licence créative complète active
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-center text-[11px] border-y border-border/30 py-2.5">
                <div>
                  <p className="text-muted-foreground uppercase tracking-wider text-[9px]">Vues générées</p>
              ...
              <div className="grid grid-cols-3 gap-1.5 pt-1">
                ...
                <Link href={`/deals/1/submission/${item.id}`} className="block w-full">
                  ...
                  >
                    Examiner <ExternalLink className="h-3 w-3" />
                  </Button>
                </Link>
              </div>
            </div>
        ...
        {filtered.length === 0 && (
          <div className="col-span-full py-16 text-center">
            ...
            <p className="text-[13px] font-medium text-foreground">Aucune ressource créative trouvée</p>
            <p className="text-[12px] text-muted-foreground">Les brouillons et soumissions de créateurs approuvés seront archivés ici automatiquement.</p>
          </div>
        )}
      </div>
    </DashboardLayout>
