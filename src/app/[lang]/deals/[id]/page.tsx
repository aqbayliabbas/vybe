...
export default function DealDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleApprove = async (subId: string, creatorName: string) => {
    try {
      ...
      if (ok) {
        toast.success(`${creatorName} approuvé !`);
        loadDealData(); // reload
      } else {
        toast.error("Échec de l'approbation de la soumission.");
      }
    } catch (e) {
      console.error(e);
  ...
  const handleDecline = async (subId: string, creatorName: string) => {
    try {
      ...
      if (ok) {
        toast.info(`${creatorName} refusé.`);
        loadDealData(); // reload
      } else {
        toast.error("Échec du refus de la soumission.");
      }
    } catch (e) {
      console.error(e);
      toast.error("Une erreur s'est produite.");
  ...
  if (loading) return (
    <DashboardLayout>
      <div className="flex flex-col items-center justify-center h-64 gap-2 text-muted-foreground">
        ...
        <p className="text-xs">Chargement des détails de l'offre...</p>
      </div>
    </DashboardLayout>
  );
  ...
  if (!deal) return (
    <DashboardLayout>
      <div className="flex flex-col items-center justify-center h-64 gap-4">
        ...
        <p className="text-foreground font-medium">Offre non trouvée</p>
        <Link href="/deals"><Button variant="outline" className="rounded-full gap-2"><ArrowLeft className="h-4 w-4" />Retour aux offres</Button></Link>
      </div>
    </DashboardLayout>
  );
    ...
    <DashboardLayout>
      <div className="mb-6">
        <Link href="/deals" className="inline-flex items-center gap-1.5 text-[12px] font-medium text-muted-foreground hover:text-foreground mb-4 transition-colors">
          <ArrowLeft className="h-3.5 w-3.5" /> Toutes les offres
        </Link>
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <Link href="/deals/new">
            <Button variant="outline" size="sm" className="rounded-full glass border-border/60 text-[12px]">Modifier l'offre</Button>
          </Link>
        </div>
      </div>
      ...
      <div className="mb-8 grid gap-3 md:grid-cols-4">
        {[
          { icon: Users, label: 'Candidatures', value: submissions.length.toString() },
          { icon: Check, label: 'Approuvé', value: approved.length.toString() },
          { icon: Eye, label: 'Vues totales', value: formatNumber(submissions.reduce((s, x) => s + x.views, 0)) },
          { icon: DollarSign, label: 'Offre moyenne', value: submissions.length ? formatDZD(Math.round(submissions.reduce((s, x) => s + x.bid, 0) / submissions.length)) : formatDZD(0) },
        ].map(s => (
      ...
      <div className="mb-6 flex items-center gap-1 border-b border-border/40">
        {([
          { key: 'applications', label: `Candidatures (${pending.length} en attente)` },
          { key: 'approved', label: `Approuvé (${approved.length})` },
        ] as const).map(t => (
        ...
        <div className="ml-auto pb-2 relative">
          ...
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Rechercher des créateurs…"
        ...
        <div className="rounded-3xl border border-border/40 bg-white/90 shadow-card">
          <div className="flex items-center gap-1.5 px-6 py-4 border-b border-border/30">
            {(['all', 'pending', 'approved', 'declined'] as const).map(f => {
              const tabLabels: Record<string, string> = { 'all': 'Tous', 'pending': 'En attente', 'approved': 'Approuvé', 'declined': 'Refusé' };
              return (
          ...
          <div className="divide-y divide-border/30">
            {filtered.map(s => (
              <div key={s.id} className="flex flex-wrap items-center gap-4 px-6 py-5 hover:bg-vybe/[0.02] transition-colors">
                ...
                <div className="flex items-center gap-6 text-[12px]">
                  <div className="text-center"><p className="font-semibold text-foreground">{formatNumber(s.followers)}</p><p className="text-muted-foreground">abonnés</p></div>
                  <div className="text-center"><p className="font-semibold text-foreground">{formatNumber(s.avgViews)}</p><p className="text-muted-foreground">vues moy.</p></div>
                  ...
                  <div className="text-center"><p className="font-semibold text-vybe-dark">{formatDZD(s.bid)}</p><p className="text-muted-foreground">offre</p></div>
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <Link href={`/deals/${id}/submission/${s.id}`}>
                    <Button variant="ghost" size="sm" className="h-8 rounded-full px-3 text-[12px] text-vybe-dark hover:bg-vybe/10 gap-1">
                      Examiner <ChevronRight className="h-3 w-3" />
                    </Button>
                  </Link>
                </div>
              </div>
            ...
            {filtered.length === 0 && (
              <div className="px-6 py-16 text-center">
                ...
                <p className="text-[13px] font-medium text-foreground">Aucune candidature pour le moment</p>
                <p className="text-[12px] text-muted-foreground">Les créateurs correspondant à votre ciblage trouveront et postuleront à cette offre.</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="divide-y divide-border/30">
            {approved.map(s => (
              <div key={s.id} className="flex flex-wrap items-center gap-4 px-6 py-5 hover:bg-vybe/[0.02] transition-colors">
                ...
                <div className="flex items-center gap-2 text-[12px]">
                  <span className="rounded-full bg-success/10 border border-success/20 px-3 py-1 font-medium text-success">Approuvé</span>
                  <span className="text-muted-foreground">En attente de soumission</span>
                </div>
                <Link href={`/deals/${id}/submission/${s.id}`}>
                  <Button variant="ghost" size="sm" className="h-8 rounded-full px-3 text-[12px] text-vybe-dark hover:bg-vybe/10 gap-1">
            ...
            {approved.length === 0 && (
              <div className="px-6 py-16 text-center">
                ...
                <p className="text-[13px] font-medium text-foreground">Aucun créateur approuvé pour le moment</p>
                <p className="text-[12px] text-muted-foreground">Approuvez les candidatures dans l'onglet Candidatures.</p>
              </div>
            )}
          </div>
        </div>
