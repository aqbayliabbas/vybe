...
const STEPS = ['Brief', 'Ciblage', 'Conditions', 'Réviser & Publier'];
const niches = ['Beauty', 'Fashion', 'Tech', 'Gaming', 'Food', 'Fitness', 'Travel', 'Finance', 'Parenting', 'Music', 'Comedy', 'Education'];
const TIPS = [
  'Une spécification claire du livrable permet d\'obtenir de meilleures soumissions',
  'Définissez la fourchette d\'abonnés pour contrôler le niveau des créateurs',
  'Laissez les créateurs proposer leurs tarifs pour les meilleurs profils',
  'Limitez le nombre max de créateurs entre 5 et 15 pour une revue gérable',
  'Ajoutez la politique de révision d\'avance pour fixer les attentes',
];
...
export default function NewDealPage() {
  ...
  const budgetDisplay = budgetType === 'fixed' ? `${fixedBudget.toLocaleString()} DZD` : budgetType === 'range' ? `${budgetMin.toLocaleString()}–${budgetMax.toLocaleString()} DZD` : 'Le créateur propose un tarif';
  ...
  const launch = async () => {
    ...
    try {
      ...
      toast.success('Offre publiée ! Les créateurs peuvent maintenant la découvrir et postuler.');
      router.push('/deals');
    } catch (e) {
      console.error(e);
      toast.error('Échec de la publication de l\'offre. Veuillez vérifier les détails.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <DashboardLayout>
      <div className="mb-8 flex items-end justify-between gap-4">
        <div>
          ...
          <h1 className="font-heading mt-1 text-[28px] font-bold tracking-tight text-foreground">Publier une offre</h1>
          <p className="text-[13px] text-muted-foreground">Sélectionnez des créateurs. Ne payez que lorsque vous approuvez le contenu.</p>
        </div>
        ...
        <div className="min-w-0 flex-1 space-y-5">
          ...
          {step === 0 && (
            <div className="rounded-3xl border border-border/60 bg-white/90 shadow-card p-7 space-y-5">
              <div>
                <label className="mb-1.5 block text-sm font-medium text-foreground">Titre de l'offre</label>
                <Input placeholder="ex., Lancement de la collection Ramadan" value={title} onChange={e => setTitle(e.target.value)} />
              </div>
              <div>
                <label className="mb-1.5 block text-sm font-medium text-foreground">Description du produit</label>
                <Textarea placeholder="Décrivez votre produit et ce qui le rend spécial…" value={description} onChange={e => setDescription(e.target.value)} rows={3} />
              </div>
              <div>
                <label className="mb-1.5 block text-sm font-medium text-foreground">Spécifications du livrable</label>
                <Textarea placeholder="Que doit exactement produire le créateur ? Format, longueur, éléments requis ?" value={deliverable} onChange={e => setDeliverable(e.target.value)} rows={3} />
              </div>
              <div className="grid gap-5 md:grid-cols-2">
                <div>
                  <div className="mb-2 flex items-center justify-between">
                    <label className="block text-sm font-medium text-foreground">À faire</label>
                    <button onClick={() => setDos(p => Array.from(new Set([...p, 'Montrer le produit clairement', 'Moins de 60 secondes'])))} className="text-[11px] font-medium text-vybe-dark bg-transparent border-0 cursor-pointer flex items-center gap-1"><Sparkles className="h-3 w-3" /> Suggérer</button>
                  </div>
                  <div className="mb-2 flex gap-2">
                    <Input placeholder="Ajouter une directive…" value={doInput} onChange={e => setDoInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && addDo()} />
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-foreground">Ressources <span className="text-muted-foreground/60">(optionnel)</span></label>
                <div onClick={() => setFiles(p => [...p, `asset_${p.length + 1}.jpg`])} className="cursor-pointer rounded-2xl border-2 border-dashed border-border/60 p-8 text-center hover:border-vybe/40 hover:bg-vybe/[0.02] transition-all">
                  ...
                  <p className="text-sm text-muted-foreground">Cliquez pour télécharger des images du produit ou des directives de marque</p>
                </div>
          ...
          {step === 1 && (
            <div className="rounded-3xl border border-border/60 bg-white/90 shadow-card p-7 space-y-6">
              <div>
                <label className="mb-2 block text-sm font-medium text-foreground">Plateforme</label>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-foreground">Fourchette d'abonnés</label>
              ...
              <div>
                <div className="mb-2 flex items-center justify-between">
                  <label className="block text-sm font-medium text-foreground">Tags de niche</label>
                  <span className="text-[11px] text-muted-foreground">{selectedNiches.length} sélectionnés</span>
                </div>
                <div className="flex flex-wrap gap-2">
                  {niches.map(n => (
          ...
          {step === 2 && (
            <div className="rounded-3xl border border-border/60 bg-white/90 shadow-card p-7 space-y-6">
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-foreground">Date limite de candidature</label>
                  <Input type="date" value={deadline} onChange={e => setDeadline(e.target.value)} />
                </div>
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-foreground">Nombre max de créateurs à accepter</label>
                  <Input type="number" value={maxCreators} min={1} max={100} onChange={e => setMaxCreators(Number(e.target.value))} />
                </div>
              </div>
              <div>
                <label className="mb-2 block text-sm font-medium text-foreground">Modèle de budget</label>
                <div className="grid grid-cols-3 gap-3">
                  {([
                    { key: 'fixed', label: 'Tarif fixe', desc: 'Même tarif pour tous' },
                    { key: 'range', label: 'Fourchette', desc: 'Min–max par créateur' },
                    { key: 'creator', label: 'Le créateur propose', desc: 'Ils fixent leur prix' },
                  ] as const).map(m => (
              ...
              {budgetType === 'fixed' && (
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-foreground">Frais par vidéo (DZD)</label>
                  <Input type="number" value={fixedBudget} onChange={e => setFixedBudget(Number(e.target.value))} />
                </div>
              )}
              {budgetType === 'range' && (
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-foreground">Fourchette de budget par vidéo (DZD)</label>
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-foreground">Politique de révision</label>
          ...
          {step === 3 && (
            <div className="rounded-3xl border border-border/60 bg-white/90 shadow-card p-7 space-y-5">
              <h2 className="font-heading text-lg font-semibold text-foreground">Révisez votre offre</h2>
              <div className="space-y-3">
                {[
                  { label: 'Titre', value: title || '—' },
                  { label: 'Plateformes', value: platforms.join(', ') || '—' },
                  { label: 'Région', value: region },
                  { label: 'Fourchette d\'abonnés', value: followerRange },
                  { label: 'Niches', value: selectedNiches.join(', ') || 'Toutes' },
                  { label: 'Budget', value: budgetDisplay },
                  { label: 'Créateurs max', value: maxCreators.toString() },
                  { label: 'Date limite', value: deadline || '—' },
                  { label: 'Révisions autorisées', value: revisions === 0 ? 'Aucune' : `${revisions}` },
                ].map(r => (
              ...
              <div className="rounded-2xl border border-border/30 bg-muted/30 p-4">
                <p className="text-[12px] text-muted-foreground">
                  <strong className="text-foreground">Aucun paiement aujourd'hui.</strong> Vous êtes facturé uniquement lorsque vous approuvez la soumission d'un créateur. Le montant du paiement est l'offre du créateur plus des frais de plateforme de 10 %.
                </p>
              </div>
            </div>
          )}
