...
export default function OnboardingStep3() {
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      toast.success('Onboarding complete! Welcome to Vybe 🚀');
      router.push('/dashboard');
    } catch (err: any) {
      console.error(err);
      ...
      <div className="relative w-full max-w-lg mx-auto px-6 py-12">
        ...
        <div className="rounded-3xl glass border border-border/30 shadow-soft p-8">
          <div className="text-center mb-8">
            ...
            <h1 className="font-heading text-2xl font-bold text-foreground">Contact & socials</h1>
            <p className="mt-2 text-[13px] text-muted-foreground">How can creators and our team reach you?</p>
          </div>

          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-foreground mb-1.5">Business phone <span className="text-destructive">*</span></label>
              <Input placeholder="+213 555 12 34 56" value={phone} onChange={e => setPhone(e.target.value)} disabled={saving} />
            </div>

            <div>
              <label className="block text-sm font-medium text-foreground mb-1.5">Contact email <span className="text-destructive">*</span></label>
              ...
              <p className="mt-1 text-[11px] text-muted-foreground">Used for business communications and invoices</p>
            </div>

            <div className="pt-3">
              <p className="text-sm font-medium text-foreground mb-1">Social media <span className="text-muted-foreground/60">(optional)</span></p>
              <p className="text-[11px] text-muted-foreground mb-3">Help creators discover and verify your brand</p>
