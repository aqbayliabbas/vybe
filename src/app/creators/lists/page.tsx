...
export default function CreatorListsPage() {
  ...
  const handleCreateList = async () => {
    ...
    try {
      ...
      toast.success("List created!", {
        description: `New empty list '${newList.name}' has been added.`,
      });
    } catch (e) {
      console.error(e);
  ...
  const handleRemoveCreatorFromList = async (creatorId: string, creatorName: string) => {
    ...
    try {
      ...
      toast.info("Creator removed", {
        description: `${creatorName} removed from list.`,
      });
    } catch (e) {
      console.error(e);
  ...
  const handleBulkInvite = () => {
    ...
    toast.success("Bulk Invites Sent!", {
      description: `Sent campaign invitations to all ${activeList.creators.length} creators in this list.`,
    });
  };

  const engagementLabel = (eng: number) => {
    if (eng >= 5) return 'Excellent';
    if (eng >= 3.5) return 'Good';
    if (eng >= 2) return 'Average';
    return 'Low';
  };
    ...
    <DashboardLayout>
      <div className="mb-6">
        <Link href="/creators" className="inline-flex items-center gap-1.5 text-[12px] font-medium text-muted-foreground hover:text-foreground mb-4 transition-colors">
          <ArrowLeft className="h-3.5 w-3.5" /> Back to Creator Search
        </Link>
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-muted-foreground/60">Collections</p>
            <h1 className="font-heading mt-2 text-[28px] font-bold tracking-tight text-foreground md:text-[34px]">Saved Creator Lists</h1>
            <p className="mt-1 text-[13px] text-muted-foreground">Organize creators into segments and invite them in bulk.</p>
          </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center h-64 gap-2 text-muted-foreground">
          ...
          <p className="text-xs">Loading your lists...</p>
        </div>
      ) : (
        <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
          {/* Lists Sidebar */}
          <div className="space-y-4">
            <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground px-2">Your lists ({lists.length})</p>
            ...
            <div className="flex flex-col gap-2">
              {lists.map(list => (
                ...
                >
                  ...
                  <p className="text-[10px] text-muted-foreground mt-2">Created {list.created}</p>
                </button>
              ))}
              ...
              {lists.length === 0 && (
                <div className="rounded-2xl border border-dashed border-border/60 p-6 text-center">
                  ...
                  <p className="text-[12px] text-muted-foreground">No lists yet</p>
                </div>
              )}
            </div>
          </div>
          ...
          {activeList ? (
            <div className="space-y-6">
              ...
              <div className="rounded-3xl border border-border/40 bg-white/90 p-6 shadow-card">
                <div className="flex flex-wrap items-start justify-between gap-4 border-b border-border/30 pb-5">
                  <div>
                    ...
                    <p className="text-[12px] text-muted-foreground mt-1">Contains {activeList.creators.length} hand-picked creators.</p>
                  </div>
                  <div className="flex gap-2">
                    ...
                    >
                      <Mail className="h-3.5 w-3.5" /> Bulk Invite list
                    </Button>
                    <Button 
                      variant="outline" 
                ...
                <div className="mt-5 grid grid-cols-2 sm:grid-cols-3 gap-4">
                  <div className="rounded-2xl bg-muted/30 p-3.5 border border-border/20">
                    <p className="text-[10px] text-muted-foreground uppercase tracking-wider flex items-center gap-1"><Users className="h-3.5 w-3.5" /> Aggregate Reach</p>
                    <p className="font-heading text-lg font-bold text-foreground mt-1">{fmt(aggregateStats.followers)}</p>
                  </div>
                  <div className="rounded-2xl bg-muted/30 p-3.5 border border-border/20">
                    <p className="text-[10px] text-muted-foreground uppercase tracking-wider flex items-center gap-1"><Eye className="h-3.5 w-3.5" /> Collective Avg Views</p>
                  ...
                  <div className="rounded-2xl bg-muted/30 p-3.5 border border-border/20 col-span-2 sm:col-span-1">
                    <p className="text-[10px] text-muted-foreground uppercase tracking-wider flex items-center gap-1"><TrendingUp className="h-3.5 w-3.5" /> Engagement Strength</p>
              ...
              <div className="rounded-3xl border border-border/40 bg-white/90 shadow-card overflow-hidden">
                <div className="px-6 py-5 border-b border-border/30">
                  <h3 className="font-heading text-sm font-semibold text-foreground">List members</h3>
                </div>
                ...
                <div className="divide-y divide-border/30">
                  {activeList.creators.map(c => (
                    <div key={c.id} className="flex flex-wrap items-center gap-4 px-6 py-4 hover:bg-vybe/[0.01] transition-colors">
                      ...
                      <div className="flex items-center gap-6 text-[11px] sm:text-xs">
                        <div className="text-center"><p className="font-semibold text-foreground">{fmt(c.followers)}</p><p className="text-muted-foreground text-[10px]">followers</p></div>
                        <div className="text-center"><p className="font-semibold text-foreground">{fmt(c.avgViews)}</p><p className="text-muted-foreground text-[10px]">avg views</p></div>
                        <div className="text-center"><p className="font-semibold text-foreground">{c.location}</p><p className="text-muted-foreground text-[10px]">city</p></div>
                      </div>
                  ...
                  {activeList.creators.length === 0 && (
                    <div className="px-6 py-16 text-center">
                      ...
                      <p className="text-[13px] font-medium text-foreground">List is empty</p>
                      <p className="text-[12px] text-muted-foreground mt-1">Add creators from the search directory.</p>
                      <Link href="/creators" className="mt-4 inline-block">
                        <Button size="sm" className="rounded-full text-[12px]">Search Creators</Button>
                      </Link>
                    </div>
                  )}
                </div>
            ...
            <div className="rounded-3xl border border-border/40 bg-white/90 p-12 text-center shadow-card">
              ...
              <p className="text-[13px] font-medium text-foreground">No lists created yet</p>
              <p className="text-[12px] text-muted-foreground mt-1">Get started by creating a new segment.</p>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="w-full max-w-[400px] rounded-3xl border border-border/45 bg-white p-6 shadow-elevated">
            <h3 className="font-heading text-[16px] font-bold text-foreground">Create Creator Collection</h3>
            <p className="text-[12px] text-muted-foreground mt-1">Group your target influencers for quick tracking and campaigns.</p>
            
            <input 
              value={newListName}
              onChange={e => setNewListName(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && handleCreateList()}
              placeholder="e.g. Algiers Food Vloggers"
