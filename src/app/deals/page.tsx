    ...
    <DashboardLayout>
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="font-heading mt-2 text-[28px] font-bold tracking-tight text-foreground md:text-[34px]">Deals</h1>
          <p className="mt-1 text-[13px] text-muted-foreground">Hand-pick creators. Pay only for content you approve.</p>
        </div>
      ...
      <div className="mb-8 grid gap-4 md:grid-cols-3">
        {[
          { icon: DollarSign, label: 'Avg Budget Pool', value: formatDZD(Math.round(totalBudget)), sub: `${deals.length} deals`, color: 'from-[oklch(0.92_0.06_300)] to-[oklch(0.96_0.04_330)]' },
          { icon: Users, label: 'Applications', value: totalSubs.toString(), sub: 'creator applications', color: 'from-[oklch(0.93_0.05_330)] to-[oklch(0.96_0.04_300)]' },
          { icon: Handshake, label: 'Active Deals', value: deals.filter(d => d.status === 'live').length.toString(), sub: 'currently open', color: 'from-[oklch(0.92_0.06_280)] to-[oklch(0.96_0.04_320)]' },
        ].map(k => (
      ...
      <div className="rounded-3xl border border-border/40 bg-white/90 shadow-card">
        <div className="flex flex-wrap items-center justify-between gap-3 px-6 py-5">
          <div>
            <h3 className="font-heading text-base font-semibold text-foreground">Deal list</h3>
            <p className="text-[12px] text-muted-foreground">{filtered.length} of {deals.length} deals</p>
          </div>
          <div className="flex items-center gap-1.5">
            {(['all', 'live', 'draft', 'ended'] as const).map(f => (
              <button key={f} onClick={() => setStatus(f)}
                ...
                {f === 'all' ? `All (${deals.length})` : f.charAt(0).toUpperCase() + f.slice(1)}
              </button>
            ))}
          </div>
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-y border-border/40 bg-muted/30">
                  {['Deal', 'Status', 'Applications', 'Budget', 'Platforms', 'Days Left', ''].map(h => (
              ...
              <tbody>
                    ...
                    <td className="px-6 py-5">
                      {c.status === 'live' ? (
                        <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-semibold ${c.daysLeft <= 5 ? 'bg-warning/10 text-warning-foreground' : 'bg-muted text-muted-foreground'}`}>
                          <Clock className="h-2.5 w-2.5" /> {fmt(c.daysLeft)}d
                        </span>
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan={7} className="px-6 py-16 text-center">
                    ...
                    <p className="text-[13px] font-medium text-foreground">No deals yet</p>
                    <p className="text-[12px] text-muted-foreground">Post your first deal to start receiving creator applications.</p>
                  </td></tr>
                )}
              </tbody>
            </table>
