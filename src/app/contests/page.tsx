  ...
  return (
    <DashboardLayout>
      ...
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-muted-foreground/60">Campaigns</p>
          <h1 className="font-heading mt-2 text-[28px] font-bold tracking-tight text-foreground md:text-[34px]">Contests</h1>
          <p className="mt-1 text-[13px] text-muted-foreground">Post a prize pool. Creators compete. You own every video.</p>
        </div>
        <div className="flex items-center gap-2">
          <div className="relative">
            <Search className="absolute left-3.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground/60" />
            <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search contests…"
      ...
      <div className="mb-8 grid gap-4 md:grid-cols-3">
        {[
          { icon: Trophy, label: 'Total Prize Pool', value: formatDZD(totalPrize), sub: `${contests.length} contests`, color: 'from-[oklch(0.92_0.06_300)] to-[oklch(0.96_0.04_330)]' },
          { icon: Eye, label: 'Total Views', value: formatNumber(totalViews), sub: 'across all contests', color: 'from-[oklch(0.93_0.05_330)] to-[oklch(0.96_0.04_300)]' },
          { icon: Users, label: 'Submissions', value: totalSubs.toString(), sub: 'creator videos', color: 'from-[oklch(0.92_0.06_280)] to-[oklch(0.96_0.04_320)]' },
        ].map((k, i) => (
      ...
      <div className="rounded-3xl border border-border/40 bg-white/90 shadow-card">
        <div className="flex flex-wrap items-center justify-between gap-3 px-6 py-5">
          <div>
            <h3 className="font-heading text-base font-semibold text-foreground">Contest list</h3>
            <p className="text-[12px] text-muted-foreground">{filtered.length} of {contests.length} contests</p>
          </div>
          <div className="flex items-center gap-1.5">
            {(['all', 'live', 'draft', 'ended'] as const).map(f => (
              <button key={f} onClick={() => setStatus(f)}
                ...
                {f === 'all' ? `All (${contests.length})` : `${f.charAt(0).toUpperCase() + f.slice(1)}`}
              </button>
            ))}
          </div>
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-y border-border/40 bg-muted/30">
                  {['Contest', 'Status', 'Submissions', 'Views', 'Prize Pool', 'Days Left', ''].map(h => (
              ...
              <tbody>
                    ...
                    <td className="px-6 py-5">
                      {c.status === 'live' ? (
                        <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-semibold ${c.daysLeft <= 5 ? 'bg-warning/10 text-warning-foreground' : 'bg-muted text-muted-foreground'}`}>
                          <Clock className="h-2.5 w-2.5" /> {fmt(c.daysLeft)}d left
                        </span>
                    ...
                    <td className="px-6 py-5 text-right">
                      <Link href={`/contests/${c.id}`}>
                        <Button variant="ghost" size="sm" className="h-8 rounded-full px-3 text-[12px] font-medium text-vybe-dark hover:bg-vybe/10">
                          Leaderboard <ArrowUpRight className="ml-1 h-3 w-3" />
                        </Button>
                      </Link>
                    </td>
                  </tr>
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan={7} className="px-6 py-16 text-center">
                    ...
                    <p className="text-[13px] font-medium text-foreground">No contests yet</p>
                    <p className="text-[12px] text-muted-foreground">Create your first contest to get started.</p>
                  </td></tr>
                )}
              </tbody>
            </table>
