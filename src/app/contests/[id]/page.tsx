  ...
  if (loading) return (
    <DashboardLayout>
      <div className="flex flex-col items-center justify-center h-64 gap-2 text-muted-foreground">
        ...
        <p className="text-xs">Loading contest details...</p>
      </div>
    </DashboardLayout>
  );
  ...
  if (!contest) return (
    <DashboardLayout>
      <div className="flex flex-col items-center justify-center h-64 gap-4">
        ...
        <p className="text-foreground font-medium">Contest not found</p>
        <Link href="/contests"><Button variant="outline" className="rounded-full gap-2"><ArrowLeft className="h-4 w-4" />Back to Contests</Button></Link>
      </div>
    </DashboardLayout>
  );
  ...
  return (
    <DashboardLayout>
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-[13px] text-muted-foreground">{contest.brand} · Prize pool {formatDZD(contest.prizePool ?? 0)} · {contest.platforms.join(', ')}</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 rounded-full border border-success/30 bg-success/10 px-3 py-1.5">
              <span className="h-2 w-2 rounded-full bg-success animate-pulse" />
              <span className="text-[11px] font-bold uppercase tracking-wider text-success">LIVE</span>
            </div>
            <div className="rounded-full border border-border/40 bg-white/80 px-4 py-2 font-mono text-[12px] font-semibold text-foreground shadow-card">
              Ends in {time.d}d {String(time.h).padStart(2, '0')}:{String(time.m).padStart(2, '0')}:{String(time.s).padStart(2, '0')}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mb-8 grid gap-3 md:grid-cols-4">
        {[
          { icon: Users, label: 'Creators', value: submissions.length.toString() },
          { icon: Eye, label: 'Total Views', value: formatNumber(submissions.reduce((s, x) => s + x.views, 0)) },
          { icon: DollarSign, label: 'Prize Pool', value: formatDZD(contest.prizePool ?? 0) },
          { icon: Trophy, label: 'Days Left', value: `${contest.daysLeft}d` },
        ].map(s => (
      ...
      <div className="mb-6 flex items-center gap-1 border-b border-border/40">
        {(['leaderboard', 'submissions', 'brief'] as Tab[]).map(t => (
          <button key={t} onClick={() => setTab(t)} className={`px-4 py-3 text-[13px] font-medium capitalize transition-colors border-b-2 -mb-px ${tab === t ? 'border-vybe text-vybe-dark' : 'border-transparent text-muted-foreground hover:text-foreground'}`}>
            {t === 'leaderboard' ? 'Leaderboard' : t === 'submissions' ? 'Submissions' : 'Brief'}
          </button>
        ))}
        <div className="ml-auto flex items-center gap-2 pb-2">
          <button onClick={handleRefresh} className="flex items-center gap-1.5 text-[11px] text-muted-foreground hover:text-foreground transition-colors bg-transparent border-0 cursor-pointer">
            <RefreshCw className="h-3 w-3" /> Refreshed {lastRefresh}
          </button>
        </div>
      </div>
        ...
        <div className="space-y-8">
          ...
          {podiumOrder.length > 0 ? (
                ...
                return (
                  <div key={entry.rank} className="flex flex-col items-center">
                    ...
                    <div className={`flex ${heights[i] || 'h-32'} w-40 flex-col items-center justify-center rounded-3xl border border-border/40 bg-gradient-to-b ${gradients[i] || gradients[0]} px-3 shadow-soft relative overflow-hidden`}>
                      ...
                      <p className="relative z-10 text-[11px] text-muted-foreground">{formatNumber(entry.views)} views</p>
            ...
            <div className="py-12 text-center text-muted-foreground">
              <p className="text-sm">Standings will be updated once creators submit videos.</p>
            </div>
          )}
          ...
          {leaderboard.length > 0 && (
            <div className="rounded-3xl border border-border/40 bg-white/90 shadow-card">
              <div className="px-6 py-5">
                <h3 className="font-heading text-base font-semibold text-foreground">Full Rankings</h3>
                <p className="text-[12px] text-muted-foreground">Live standings — updated every 6 hours via Phyllo</p>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-y border-border/40 bg-muted/30">
                      {['Rank', 'Creator', 'Views', 'Prize', '24h Change'].map(h => (
      ...
      {tab === 'submissions' && (
        <div className="rounded-3xl border border-border/40 bg-white/90 shadow-card">
          <div className="px-6 py-5">
            <h3 className="font-heading text-base font-semibold text-foreground">Submitted Videos</h3>
            <p className="text-[12px] text-muted-foreground">{submissions.length} videos received</p>
          </div>
          {submissions.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-y border-border/40 bg-muted/30">
                    {['Creator', 'Platform', 'Views', 'Submitted', 'Status', ''].map(h => (
                      <th key={h} className="px-6 py-3 text-left text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/70">{h}</th>
                    ))}
                  </tr>
                </thead>
      ...
      {tab === 'brief' && (
        <div className="rounded-3xl border border-border/40 bg-white/90 shadow-card p-7 space-y-6 max-w-2xl">
          ...
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground/60 mb-2">Platforms</p>
