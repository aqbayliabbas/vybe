...
export default function DashboardPage() {
  ...
  const kpis = useMemo(() => {
    ...
    return [
      { label: 'Active Campaigns', value: active.toString(), delta: `+${active}`, progress: campaigns.length ? (active / campaigns.length) * 100 : 0, icon: Megaphone },
      { label: 'Total Submissions', value: totalSubs.toString(), delta: `+${totalSubs}`, progress: totalSubs ? 100 : 0, icon: TrendingUp },
      { label: 'Total Views', value: formatNumber(totalViews), delta: '+0%', progress: totalViews ? 100 : 0, icon: Eye },
      { label: 'Total Spent', value: formatDZD(totalSpent), delta: '+0%', progress: totalSpent ? 100 : 0, icon: DollarSign },
    ];
  }, [campaigns]);
  ...
  return (
    <DashboardLayout>
      ...
      <div className="mb-10 flex flex-wrap items-center justify-between gap-4">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-muted-foreground/60">Overview</p>
          <h1 className="font-heading mt-2 text-[28px] font-bold tracking-tight text-foreground md:text-[34px]">
            Good morning, Ahmed
          </h1>
          <p className="mt-1 text-[13px] text-muted-foreground">Here's what's happening with your campaigns today.</p>
        </div>
      ...
      <div className="rounded-3xl border border-border/40 bg-white/90 shadow-card">
        <div className="flex flex-wrap items-center justify-between gap-3 px-6 py-5">
          <div>
            <h3 className="font-heading text-base font-semibold text-foreground">Your Campaigns</h3>
            <p className="text-[12px] text-muted-foreground">Manage and track all your active campaigns</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="p-12 text-center text-muted-foreground">
            ...
            <p className="text-xs mt-1">Create a new contest or deal to get started!</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-y border-border/40 bg-muted/30">
                  {['Campaign', 'Type', 'Status', 'Submissions', 'Views', 'Budget', 'Created', ''].map(h => (
