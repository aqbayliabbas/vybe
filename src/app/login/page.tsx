...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const friendlyMsg = isFetchError
        ? 'Failed to connect to Supabase. Please ensure your NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY are set correctly in your .env.local file.'
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const isFetchError = err.message === 'Failed to fetch' || err.name === 'AuthRetryableFetchError' || (err.message && err.message.includes('fetch'));
      const friendlyMsg = isFetchError
        ? 'Failed to connect to Supabase. Check your .env.local credentials.'
    ...
    <div className="min-h-screen bg-background flex items-center justify-center relative overflow-hidden">
      ...
      <div className="relative w-full max-w-md mx-auto px-6">
        ...
        <div className="rounded-3xl glass border border-border/30 shadow-soft p-8">
          ...
          <p className="mt-2 text-[13px] text-muted-foreground text-center">Sign in to your brand account</p>
          ...
          <form onSubmit={handleLogin} className="mt-8 space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <Link href="/forgot-password" className="text-[12px] font-medium text-vybe-dark hover:underline">Forgot password?</Link>
              </div>
            ...
            {errorMsg && (
              <div className="text-[12px] text-red-500 bg-red-500/10 p-3.5 rounded-xl border border-red-500/20 text-center font-medium leading-relaxed">
                {errorMsg}
              </div>
            )}
          ...
          <div className="mt-6 relative">
            ...
            <div className="relative flex justify-center"><span className="bg-background px-3 text-[11px] text-muted-foreground/70">or</span></div>
          </div>
          ...
          <Button 
            ...
            disabled={loading || googleLoading}
            className="w-full mt-4 h-11 rounded-2xl text-[13px] font-medium glass border-border/40"
          >
            {googleLoading ? (
          ...
          <p className="mt-6 text-center text-[12px] text-muted-foreground">
            Don&apos;t have an account? <Link href="/signup" className="font-semibold text-vybe-dark hover:underline">Sign up free</Link>
          </p>
        </div>
      </div>
    </div>
